...
// n이 undefined이거나 음수인 경우, 빈 배열을 리턴합니다. --조건문
// n이 배열의 길이를 벗어날 경우, 전체 배열을 shallow copy한 새로운 배열을 리턴합니다. --3가지 메서드 slice,
...
_.uniq = function (arr) { //[1,2,3,4,1]
  ...
  //result ==> (1,2)
  //indexOf를 사용해서_.indexOf(result,2)
};
...
_.map = function (arr, iteratee) {
  ...
  //each와 매우 비슷하지만 interatee(배열요소) --> 이걸 실행한 결과를 모든 새로운 배열을 반환한다.
};
...
_.pluck = function (arr, keyOrIdx) {
  ...
  //[[1,2] [2,3] [4,2]] 이 배열의 요소에 1번째 인덱스들만 모아서 배열로 만들어 반환
  //[2,3,2]
};
...
_.reduce = function (arr, iteratee, initVal) { // [1, 2, 3, 4]
  // TODO: 여기에 코드를 작성합니다.
  let acc = initVal
  let result;
  ...
  if (acc === undefined) {
    result = _.slice(arr);
    acc = result.shift();
    _.each(result, (ele) => { acc = iteratee(acc, ele, _.indexOf(result, ele), result); })
  }
  else {
    _.each(arr, (ele) => { acc = iteratee(acc, ele, _.indexOf(arr, ele), arr); });
  }
  return acc;
